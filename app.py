...
def upload():
    ...
    if request.method == "POST":
        ...
        # Instantiate a browser object (e.g., Chrome)
        ...
        # Use WebDriverManager to download and install the appropriate ChromeDriver
        ...
        if data=="tuna tartare":
            url="https://ahealthylifeforme.com/tuna-tartare-recipe/"
            browser.get(url)
            # html=browser.html
            html = browser.page_source
        ...
        if (data=="greek salad" or data=="oysters" or data=="smoked scallop" or data=="paella"):    
            ...
            #     output=description[2].text
                if len(description) > 0:
                    if description[0].text != '\n':
                        output = description[0].text    
                    elif len(description) > 1 and description[1].text != '\n':
                        output = description[1].text
                    elif len(description) > 2 and description[2].text != '\n':
                        output = description[2].text
                else:
                    output = ''
